...
busnoOfSeats:number=0;
  ...
  onChange() {
    if(this.showBusDetails && this.selectedOperation == 'Display all records')
    {
this.getAllBuses();
    }

    else if(this.showDriverDetails && this.selectedOperation == 'Display all records')
    {
this.getDriverList();
    }
    ...
    else if(this.showDriverDetails && this.selectedOperation == 'Add Records')
    {
    ...
    else if(this.showBusDetails && this.selectedOperation == 'Edit Records')
    {
this.getAllBuses();
    }
    else if(this.showDriverDetails && this.selectedOperation == 'Add Records')
...
this.getAllBuses();
  ...
  let driver_Id=Number((<HTMLInputElement>document.getElementById("driverId")).value);

    this.service.updateBusDetail(busId,driver_Id)  
  .subscribe((data: any) => console.log(data), (error: any) => console.log(error));
  }
...
f.mobile=this.driverMobile;
  ...
  .subscribe((data: any) => {console.log(data); this.getDriverList()}) ;
...
public updateDriverInfo(driverId:number){
  ...
  let bus_mobile=(<HTMLInputElement>document.getElementById("driverId")).value;

  this.service.updateDriverInfo(driverId,bus_mobile)  
.subscribe((data: any) => console.log(data), (error: any) => console.log(error));
}
